...
function parseDateString(datetime) {
    ...
    if (time === undefined) {
        ...
        return new Date(Number(date_members[2]), Number(date_members[1]) - 1, Number(date_members[0]), Number(time_members[0]) + 3, Number(time_members[1]), Number(time_members[2]));
    }
}
